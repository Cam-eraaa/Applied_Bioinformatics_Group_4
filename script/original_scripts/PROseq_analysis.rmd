...
positions = FCdata[, 1:5]
chr = positions %>% dplyr::select(Chr) %>% separate(
  col = Chr,
  into = c("chr"),
  sep = ";",
  remove = TRUE
)
strand = positions %>% dplyr::select(Strand) %>% separate(
  col = Strand,
  into = c("strand"),
  sep = ";",
  remove = TRUE
...
# Ran loops to find the the max number of values for stop is 1723
...
# Splitting the values for each row
for(start_row in 1:num_rows) {
  ...
  start_min = c(start_min, min(start_matrix[start_row, ], na.rm=TRUE))
  start$start_min[start$Geneid == start[[2]][start_row]] = max(start_matrix[start_row, ], na.rm=TRUE)
}
...
for(end_row in 1:rows) {
  ...
  end_max = c(end_max, max(end_matrix[end_row, ], na.rm=TRUE))
  end$end_max[end$Geneid == end[[2]][end_row]] = max(end_matrix[end_row, ], na.rm=TRUE)
}

positions_fixed = data.frame(positions$Geneid, chr, strand)
positions_fixed = positions_fixed %>% dplyr::rename(Geneid = positions.Geneid)
positions_fixed = inner_join(positions_fixed, start)
positions_fixed = inner_join(positions_fixed, end)
positions_fixed = positions_fixed %>% dplyr::select(Geneid, chr, start_min, end_max, strand)
positions_fixed = positions_fixed %>% dplyr::rename(geneID = Geneid, Chr = chr, Start = start_min, End = end_max, Strand = strand)
geneInfo = inner_join(positions_fixed, geneInfo)
```
