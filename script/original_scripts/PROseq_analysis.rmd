...
params:
    ...
    PROseq.genes.data: PROseq.CBP.data.tsv
---
...
PROseqGenes = PROseqGenes %>% dplyr::select(geneID, Chr, Start, End)
...
loadingsDF.all.factorDistance.max.peakInfo_genes = inner_join(loadingsDF.all.factorDistance.max.peakInfo2, PROseqGenes)
fileNameProseq_genes = paste( params$workingDir, params$resultsDir,  
                        params$PROseq.genes.data,
                             sep = "/")
write.table(x = loadingsDF.all.factorDistance.max.peakInfo_genes, file = fileNameProseq_genes,
                  quote = F, sep = "\t", col.names = T, row.names = F)
