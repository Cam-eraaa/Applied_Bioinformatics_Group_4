...
vector = c()
start = positions %>% dplyr::select(Start)
...
# Ran loops to find the the max number of values for stop is 1723
end_matrix = matrix(NA, nrow = 13947, ncol =1723)
vector = c()
end = positions %>% dplyr::select(End)
rows = dim(end)[1]
end_max = c()
...
for(end_row in 1:rows) {
  ...
  end_max = c(end_max, min(end_matrix[end_row, ], na.rm=TRUE))
}
positions_fixed = data.frame(positions$Geneid, chr, start_min, end_max, strand)
positions_fixed = positions_fixed %>% dplyr::rename(geneID = positions.Geneid, Chr = chr, Start = start_min, End=end_max, Strand = strand)
