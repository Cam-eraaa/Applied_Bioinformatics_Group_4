...
params:
  ...
  results_file: results/altered_method/both.combined.AllData.withMetaData.txt
  results_file_cleaned: results/altered_method/both.AllData_cleaned.txt
...
scores = ROCplotSummary %>% dplyr::filter(score == "distance5")
...
result = AllData_test %>% dplyr::select(
  PRO_Time,
  PRO_Gd7,
  PRO_Toll10b,
  PRO_Toll9,
...
cleaned_AllData = result_unique %>% dplyr::select(geneID, ATACseq_geneID, factor, direction)
