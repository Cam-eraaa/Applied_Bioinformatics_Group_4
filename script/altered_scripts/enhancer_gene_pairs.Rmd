...
#This is the output from PROseq_analysis
...
joined$PROseq_Classification = paste(joined$Gene_Factor,joined$Gene_Direction, sep = " " )
joined$ATACseq_Classification = paste(joined$Enhancer_Factor,joined$Enhancer_Direction, sep = " " )

joined = joined %>% mutate(pair_score_Gd7 = Enhancer_Gd7+Gene_Gd7)
joined = joined %>% mutate(pair_score_Toll10b = Enhancer_Toll10b+Gene_Toll10b)
joined = joined %>% mutate(pair_score_Toll9 = Enhancer_Toll9+Gene_Toll9)
joined = joined %>% mutate(pair_score_Time = Enhancer_Time+Gene_Time)

pairs_filtered = joined %>% filter(PROseq_Classification == ATACseq_Classification)

enhancer_gene_pairs = pairs_filtered %>% dplyr::select(Enhancer_ID, Enhancer_Chr, Enhancer_Start, Enhancer_End, Enhancer_Factor, Gene_geneID, Gene_Chr, Gene_Start, Gene_End, Gene_Factor, pair_score_Gd7, pair_score_Toll10b, pair_score_Toll9, pair_score_Time, EnhancerDistance)
