...
params:
  ...
  results_file_no_filtering: results/altered_method/from_joint_peak/enhancers_no_filtering.tsv
...
CBPpeak.ATAC.PRO.CBP  = inner_join(CBPpeak.ATAC.PRO.CBP,  overlap_ATAC_K27ac, by="geneID")
...
CBPpeak.ATAC.PRO.CBP.K27ac.enhancer = left_join(CBPpeak.ATAC.PRO.CBP.K27ac, overlap_CBP_enhancers, by="geneID")
CBPpeak.ATAC.PRO.CBP.K27ac.enhancer = left_join(CBPpeak.ATAC.PRO.CBP.K27ac.enhancer, EnhancersInfo, by="enhancerID")
...
CBPpeak.ATAC.PRO.CBP.K27ac.enhancer.inSitu = left_join(CBPpeak.ATAC.PRO.CBP.K27ac.enhancer, overlap_CBP_inSitu, by="geneID")
CBPpeak.ATAC.PRO.CBP.K27ac.enhancer.inSitu = left_join(CBPpeak.ATAC.PRO.CBP.K27ac.enhancer.inSitu, inSituInfo, by="insituID")
...
PI =  EnhancerInfo %>% filter(inSitu == 1) %>% ggplot(aes(PRO_Toll9, PRO_Toll10b, color = inSituLocation))+ggtitle("PRO-seq data for in Situ data") + geom_point() + geom_density_2d(EnhancerInfo,mapping = aes(PRO_Toll9, PRO_Toll10b),color = "grey")
AE =  EnhancerInfo %>% filter(enhancer == 1) %>% ggplot(aes(ATAC_Toll9, ATAC_Toll10b, color = enhancerGenotype)) + geom_point() + geom_density_2d(EnhancerInfo,mapping = aes(ATAC_Toll9, ATAC_Toll10b),
                                                                                                                                                  color = "grey")
...
CI =  EnhancerInfo %>% filter(inSitu == 1) %>% ggplot(aes(CBP_Toll9, CBP_Toll10b, color = inSituLocation)) + geom_point() + geom_density_2d(EnhancerInfo,
                                                                                                                                            mapping = aes(CBP_Toll9, CBP_Toll10b),
                                                                                                                                            color = "grey")
KE =  EnhancerInfo %>% filter(enhancer == 1) %>% ggplot(aes(K27ac_Toll9, K27ac_Toll10b, color = enhancerGenotype)) + geom_point() + geom_density_2d(EnhancerInfo, mapping = aes(K27ac_Toll9, K27ac_Toll10b),color = "grey")+ ggtitle("Enhancer data for H3K27ac")
KI =  EnhancerInfo %>% filter(inSitu == 1) %>% ggplot(aes(K27ac_Toll9, K27ac_Toll10b, color = inSituLocation)) + geom_point() + geom_density_2d(EnhancerInfo,mapping = aes(K27ac_Toll9, K27ac_Toll10b),color = "grey") + ggtitle("In Situ data for H3K27ac")
...
AllData_test_no_filtering = AllData %>% mutate(Enhancer_Gd7 =  ATAC_Gd7 + CBP_Gd7 + K27ac_Gd7)
AllData_test_no_filtering = AllData_test_no_filtering %>% mutate(Enhancer_Toll10b =  ATAC_Toll10b + CBP_Toll10b + K27ac_Toll10b)
AllData_test_no_filtering = AllData_test_no_filtering %>% mutate(Enhancer_Toll9 =  ATAC_Toll9 + CBP_Toll9 + K27ac_Toll9)
AllData_test_no_filtering = AllData_test_no_filtering %>% mutate(Enhancer_Time =  ATAC_Time + PRO_Time)
enhancers.no.filtering = AllData_test_no_filtering %>% dplyr::select(
  geneID,
  Chr,
  ATAC_Start,
  ATAC_End,
...
enhancers.no.filtering.unique = unique(enhancers.no.filtering)
...
write.table(
  x = enhancers.no.filtering.unique,
  file = paste(params$workingDir, params$results_file_no_filtering, sep = "/"),
  quote = F,
  sep = "\t",
  row.names = F,
  col.names = T
