---
title: "Combining the PRO-, ATAC and ChIP-seq data"
...
params:
    ...
    PROseqDir: data_johan/results/all/PROseq/
    PROseq.CDS.data: PROseq.data.tsv
    PROseq.data.PPC: PROseq.data.PPC.tsv
    ATACSeqDir: data_johan/results/all/ATACseq
    ATACSeq.data: ATACseq.data.raw.tsv
    ChIPseqDir: data_johan/results/all/chipSeq
    k27ac.data: K27ac.ChIPseq.peak.pattern.tsv
    CBP.data: ATAC-CBP_dm6.ChIPseq.peak.pattern.tsv
    sampleInfo: data_johan/information/ChIP_ATAC_sampleInfo.tab.txt
    # I am working on recreating the Regions file
    Regions: data_johan/results/all/combined.peaks.location.tsv
    bestTargets_write: results/original_method/TopTargets.txt
    bestTargets: data_johan/information/all/TopTargets.txt
    results_file: results/altered_method/combined.AllData.withMetaData.txt
    results_file_cleaned: results/altered_method/AllData_cleaned.txt
...
gtfFile = paste(params$workingDir,
                "Drosophila_melanogaster.BDGP6.28.101.gtf",
                sep = "/")
gtfInfo = read.table(
  file = gtfFile,
  header = F,
...
gtfInfoGene = gtfInfoGene %>% separate(
  col = V9,
  sep = ";",
  into =  c("geneID1", "geneName1", "gene_source", "gene_biotype", "empty")
)
gtfInfoGene = gtfInfoGene %>% separate(
  col = geneID1,
  sep = "\"",
  into =  c("irrelevant", "geneID", "empty")
)
gtfInfoGene = gtfInfoGene %>% separate(
  col = geneName1,
  sep = "\"",
  into =  c("irrelevant1", "geneName", "empty")
)
...
PROseqGenes = gtfInfoAnnotation %>% dplyr::rename(
  "Chr" = V1,
  "GeneStart" = V4,
  "GeneStop" = V5,
  "geneDir" = V7
...
gtfInfomRNA_DF = gtfInfomRNA %>% separate(
  col = V9,
  sep = ";",
  into =  c("geneID1", "transcriptID1")
)
gtfInfomRNA_DF = gtfInfomRNA_DF %>% separate(col = geneID1,
                                             sep = "\"",
                                             into =  c("irrelevant", "geneID"))
gtfInfomRNA_DF = gtfInfomRNA_DF %>% separate(
  col = transcriptID1,
  sep = "\"",
  into =  c("irrelevant1", "transcriptID")
)
...
### CDS values
```{r PROseq  CDS info}
# Load the tsv based on counts per gene
PROseqInfoCDS = read.table(
  file = paste(params$workingDir, params$PROseqDir, params$PROseq.CDS.data,
               sep = "/"),
  quote = "",
  sep = "\t",
  header = TRUE,
...
# Extract the mutants and annotation
annotation2 =  PROseqInfoCDS %>% filter(factor %in% c("Toll10b", "Toll9", "Gd7") &
                                       annotation2 == "DE") %>%
  dplyr::select(geneID, annotation, distance, factor, direction) %>%
  distinct()


PROseqData = PROseqInfoCDS %>% dplyr::select(geneID,
                                          factor,
                                          direction,
                                          annotation,
                                          annotation2,
...
PROseqData.DE  = PROseqData %>% filter(annotation2 == "DE" &
                                         factor != "Time")
...
topTarget = read.table(
  file = topTargetsFileName,
  header = T,
  sep = "\t",
  quote = "",
  stringsAsFactors = F
...
topTargetsFileName_new = paste(params$workingDir, params$bestTargets_write, sep = "/")
PROseqData.topTargets =  inner_join(PROseqData, topTarget) %>% dplyr::select(geneName, factor, direction, annotation, Factor, Direction)
write.table(
  x = PROseqData.topTargets,
  file = topTargetsFileName_new,
  sep = "\t",
  quote = F,
  row.names = F,
  col.names =  T
...
# Plot the PROseq data where it has been divided into background and factor
PROseqInfoCDS %>% dplyr::select(geneID,
                             Chr,
                             Start,
                             End,
                             Strand,
                             ...
                             annotation2,
                             Gd7,
                             Toll9) %>% distinct()  %>%  filter(factor != "Time") %>% ggplot(mapping = aes(
                               x = Gd7,
                               y = Toll9,
                               color = factor,
                               shape = direction
...
ATACseqInfo = ATACseqInfo %>% dplyr::rename(geneID = Geneid)
...
K27acInfo = K27acInfo %>% dplyr::rename(geneID = Geneid)
...
CBPInfo = CBPInfo %>% dplyr::rename(geneID = Geneid)
...
usedRegionsAndGenes = read.table(
  file = paste(params$workingDir, params$Regions,
               sep = "/"),
  header = T,
  quote = "",
  sep = "\t",
...
colnames(usedRegionsAndGenes)[2] = "K27ac_geneID"
...
PRO_ATAC_CHiP_data = AllData %>%
  ...
  summarise(
    PROseq_Time = mean(PROseq_Time),
    PROseq_Gd7 = mean(PROseq_Gd7),
    PROseq_Toll10b = mean(PROseq_Toll10b),
    PROseq_Toll9 = mean(PROseq_Toll9),
    PPC_Time = mean(PPC_Time),
    PPC_Gd7 = mean(PPC_Gd7),
    PPC_Toll10b = mean(PPC_Toll10b),
    PPC_Toll9 = mean(PPC_Toll9),
    ATACseq_Time = mean(ATACseq_Time),
    ATACseq_Gd7 = mean(ATACseq_Gd7),
    ATACseq_Toll10b = mean(ATACseq_Toll10b),
    ATACseq_Toll9 = mean(ATACseq_Toll9),
...
PRO_ATAC_CHiP_data = as.data.frame(AllData2)

PRO_ATAC_CHiP_data2 = as.data.frame(
...
mir.pca <-
  prcomp(PRO_ATAC_CHiP_data2, center = TRUE, scale = FALSE)
