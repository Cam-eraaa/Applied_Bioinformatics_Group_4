...
loadingsDF.all.filtered = loadingsDF.all.factorDistance.max %>% filter(distance >2)  %>% dplyr::select(Geneid, factor, direction)
...
fileNameATACseq = paste( params$workingDir, params$proSeqDir,  
                        params$ATACSeq.data,
                             sep = "/")
write.table(x = loadingsDF.all.filtered.peakInfo, file = fileNameATACseq,
...
nearest(chipPeakData,PROseqData,  ignore.strand=TRUE)
...
ATAC_peakInfo_link = ATAC_peakInfo_link %>% 
  rename( distance = "ATAC_distance", factor="ATAC_factor",Geneid = "ATAC_region", direction = "ATAC_direction")
