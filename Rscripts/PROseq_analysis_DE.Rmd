...
toll9_vs_10.3h.lFC1 = toll9_vs_10.3h %>% dplyr::filter(abs(log2FoldChange) > 1)  %>% dplyr::select(log2FoldChange,padj,ENSEMBL)
...
table(universe$ENSEMBL)
table(table(toll10_vs_gd7.5h.lFC1$ENSEMBL))


toll10_vs_gd7.5h.lFC1 = toll10_vs_gd7.5h.lFC1 %>% distinct(ENSEMBL, .keep_all = TRUE)
toll9_vs_gd7.5h.lFC1 = toll9_vs_gd7.5h.lFC1 %>% distinct(ENSEMBL, .keep_all = TRUE)
toll9_vs_10.5h.lFC1 = toll9_vs_10.5h.lFC1 %>% distinct(ENSEMBL, .keep_all = TRUE)
universe = universe %>% distinct(ENSEMBL, .keep_all = TRUE)
...
similair = norm.count.5h.rlog.DEG %>% dplyr::select(l2fc_toll10b_vs_gd7,l2fc_toll9_vs_gd7,l2fc_toll9_vs_toll10b,ENSEMBL)
similair[is.na(similair)] = 0 
# convert to dataframe
similair2 = similair %>% dplyr::select( l2fc_toll10b_vs_gd7,l2fc_toll9_vs_gd7,l2fc_toll9_vs_toll10b)


norm.count.5h.rlog.DEG %>% dplyr::filter(ENSEMBL == "FBgn0002733")
FBgn0002733
similair2[rowSums(similair2)>2,]
similair2[similair2 > 1] =1
similair2[similair2 < 1] =0
rownames(similair2) = similair$ENSEMBL
library(UpSetR)

upset(similair2, nsets = 3,nintersects = NA,  point.size = 3.5, line.size = 2, 
    mainbar.y.label = "Significant Correlation Intersections", sets.x.label = "Nr of sig correlations", 
...
write.table(x = gd7_3h_vs_5h, quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE, 
            file =  paste(params$workingDir, params$dataDir,"gd7.normcounts.CDS.DE.lfc1.txt", sep = "/"))
...
toll9_3h_vs_5h=left_join(norm.count.toll9.rlog, toll9_3h_vstoll9_5h.lFC1)
...
write.table(x = toll9_3h_vs_5h, quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE, 
            file =  paste(params$workingDir, params$dataDir,"toll9.normcounts.CDS.DE.lfc1.txt", sep = "/"))
...
toll10b_3h_vs_5h=left_join(norm.count.toll10b.rlog, toll10b_3h_vstoll10b_5h.lFC1)
...
write.table(x = toll10b_3h_vs_5h, quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE, 
            file =  paste(params$workingDir, params$dataDir,"toll10b.normcounts.CDS.DE.lfc1.txt", sep = "/"))
