...
params:
  ...
  dataFile: data/ChIPseq/K27me3-PROseq.counts.pmt.txt
...
sampleInfo$sampleInfo1 = gsub(pattern = "..ChIP_seq_mapped_dm6.bam_dmel." , replacement = "", sampleInfo$bamFile)
sampleInfo$sampleInfo1 = gsub(pattern = "..Zetlinger_ChIP.mapped.bam_dm6." , replacement = "", sampleInfo$sampleInfo1)
sampleInfo$sampleInfo1 = gsub(pattern = ".dmel.bowtie2.sorted.bam" , replacement = "", sampleInfo$sampleInfo1)
...
sampleInfo$sampleInfo1 = gsub(pattern = "gd7" , replacement = "Gd7", sampleInfo$sampleInfo1)
sampleInfo$sampleInfo1 = gsub(pattern = "Tol10b" , replacement = "Toll10b", sampleInfo$sampleInfo1)
sampleInfo$sampleInfo1 = gsub(pattern = "Toll910" , replacement = "Toll9", sampleInfo$sampleInfo1)
sampleInfo$sampleInfo1 = gsub(pattern = "rep" , replacement = "", sampleInfo$sampleInfo1)
sampleInfo$sampleInfo1 = gsub(pattern = "rep" , replacement = "", sampleInfo$sampleInfo1)
sampleInfo$sampleInfo1 = gsub(pattern = "WCE" , replacement = "Input", sampleInfo$sampleInfo1)

sampleInfo = sampleInfo %>% separate(col = sampleInfo1,sep = "_",into =  c( "Genotype","Type","Replicate"), remove = F)
...
sampleInfo  = sampleInfo %>% 
  dplyr::select(sampleName,Type,Genotype,Replicate,bamFile)


sampleInfo.k27me3 = sampleInfo%>% filter(Type == "K27me3")
...
countData.QC = countData[, sampleInfo$sampleName]
...
Input = normExpressionDF %>% dplyr::select(geneID,Genotype, Type,rlog) %>%
  filter(Type == "Input") %>%
  group_by(geneID,Genotype ) %>%
  summarise(meanInput = mean(rlog ))

normExpressionDF = inner_join(normExpressionDF, Input)
normExpressionDF = normExpressionDF %>% mutate(H3K27me3 = rlog - meanInput) 

ggplot(normExpressionDF, aes(x = H3K27me3, color = Genotype, linetype = Replicate)) + geom_density()+ facet_grid(.~Type)





geneInfo = normExpressionDF %>% dplyr::select(geneID,Type,  H3K27me3, sampleName) %>%
  dplyr::filter (Type != "Input") %>%
  dplyr::group_by (geneID) %>%
  dplyr::summarize ( min = min(H3K27me3), max = max(H3K27me3), mean = mean(H3K27me3), sd = sd(H3K27me3), overQC = length(which(H3K27me3 > 0.5)) ) 

geneInfo.QC2 = geneInfo %>% filter(overQC > 2)
normExpressionDF.QC = normExpressionDF %>% filter(geneID %in% geneInfo.QC2$geneID)

ggplot(normExpressionDF.QC, aes(x = H3K27me3, color = Genotype, linetype = Type)) + geom_density()


normExpression.QC = normExpressionDF.QC %>%
  dplyr::filter (Type != "Input") %>%
  dplyr::select(geneID, sampleName, H3K27me3) %>% 
  spread(key = sampleName, value = H3K27me3)
...
pctable$Genotype = as.character(pctable$Genotype )

ggplot(pctable, aes(PC1,PC2, color = Genotype))+ geom_point()

pcaPlot <- ggpairs(pctable,columns = c("PC1","PC2","PC3","Type", "Genotype") ,
                     axisLabels="internal",
                     mapping = aes(color=Type,  shape = Genotype, linetype = Genotype),
                     upper = list(continuous = "blank", combo = 'box', discrete = "blank"),
                     lower = list(continuous = "points", combo = "blank", discrete = "blank"),
                     diag  = list(continuous = "blan", discrete = "blank",progress = FALSE)
  )
