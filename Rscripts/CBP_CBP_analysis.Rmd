...
params:
  ...
  chipSeqData: CBP-CBP_dm6.featureCount.count.tab.txt 
  NormalisedChipSeqData: CBP-CBP_dm6.featureCount.rlog.tab.txt
  Antibody: ATAC-CBP_dm6
  distanceCutoff: 2
...
PROseqGenes = PROseqGenes %>% dplyr::rename("Chr"= V1, "GeneStart" = V4,"GeneStop" = V5,"geneDir" = V7)
...
normExpressionDF.QC.CBP


NormalisedCountFile = paste(params$workingDir, params$dataDir,
                         paste(params$NormalisedChipSeqData, sep = "."), sep = "/")

write.table(x = normExpressionDF.QC.CBP, file = NormalisedCountFile,
...
pdf(paste(params$workingDir, params$chipSeqDir, 
  "CBP_ATACpeak_distance_analysis.pdf", sep ="/") ,width = 5, height = 5)
...
ggsave(paste(params$workingDir, params$chipSeqDir, 
             paste( "CBP_ATACpeak_PCA_analysis.pdf", sep ="_"),
             sep = "/"))
...
fileNameFigCDSsamples = paste( params$workingDir, params$chipSeqDir, 
                        ...
                        sep = "/")
ggplot(line, mapping = aes(x = PC1,y = PC2, color = factor))+ geom_line() + geom_point(data = pctable,  mapping = aes(x = PC1,y = PC2, color = factor))+
...
fileNameChiPseq = paste( params$workingDir, params$chipSeqDir,  
...
loadingsDF.all.filtered = loadingsDF.all.factorDistance.max.peakInfo %>% filter(distance >params$distanceCutoff)  %>% dplyr::select(Geneid, factor, direction)
...
fileNameChiPseq = paste( params$workingDir, params$chipSeqDir,  
                        paste(params$Antibody,"ChIPseq.peak.pattern.tsv", sep = "."),
                             sep = "/")
write.table(x = loadingsDF.all.filtered.peakInfo, file = fileNameChiPseq,
                  quote = F, sep = "\t", col.names = T, row.names = F) 
...
# Merge the data to the proSeq data
```{r filterPeaks}
...
ChIP_peakInfo_link = ChIP_peakInfo_link %>% 
  rename(ChIPdistance =  distance , ChIP_factor = factor,ChIP_region = Geneid, ChIP_direction = direction)
...
Both_data.filtered.overlapping = Both_data %>% dplyr::filter(EnhancerDistance == 0 )  %>% filter(ChIPdistance > 2 )
...
ChIPdataGood = rbind(Both_data_same,Both_data_good) %>% filter(EnhancerDistance < 15000) %>% dplyr::select(geneID, ChIP_region,ChIP_factor,ChIP_direction, Gd7,Toll10b, Toll9) 
...
length(unique(ChIPdataGood$geneID))
