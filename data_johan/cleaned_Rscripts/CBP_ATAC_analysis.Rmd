...
params:
  ...
  chipSeqData: ATAC-CBP_dm6.featureCount.count.tab.txt
  NormalisedChipSeqData: ATAC-CBP_dm6.featureCount.rlog.tab.txt
...
loadingsDF.all.filtered = loadingsDF.all.factorDistance.max %>% filter(distance >params$distanceCutoff)  %>% dplyr::select(Geneid, factor, direction)
...
Both_data.filtered.overlapping = Both_data %>% filter(EnhancerDistance == 0 )  %>% filter(ChIP_distance > 2 )
